...
describe('Organization', () => {
    ...
    it('Create organization without title', () => {
        organization.post({name: '', statusCode: 400, statusText: "Bad Request"}).then(response => {
    ...
    it('Edit organization without name', () => {
        organization.putOrganization({name:"", statusCode: 400, statusText: "Bad Request", organizationID: organizationID}).then(response => {
            expect(response.body.name[0]).eql("The name field is required.");
        })
    })

    it('Edit organization without vacation days', () => {
        organization.putVacationDays({additionalDays: "", days: "", expirationDate: "", workingMonths: "", statusCode: 422, statusText: "Unprocessable Entity", organizationID: organizationID}).then(response => {
    ...
    it('Delete organization with wrong password', () => {
        organization.delete({passwordOrEmail: "34141", statusCode: 403, statusText: "Forbidden", organizationID: organizationID}).then(response => {
    ...
    it('Delete organization', () => {
        organization.delete({statusCode: 201, statusText: "Created", organizationID: organizationID}).then(response => {
            expect(response.body.id).eq(organizationID)
        })
    })

    it('Delete an organization that has already been deleted', () => {
        organization.delete({statusCode: 403, statusText: "Forbidden", organizationID: organizationID}).then(response => {
